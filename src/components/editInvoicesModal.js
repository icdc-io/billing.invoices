...
import { Modal, Button, Input, Dropdown, Header, Icon } from 'semantic-ui-react';
    ...
    return (
        <Modal size='tiny' open={open} className='billing-modal'>
            <Modal.Content>
                ...
                <Header as='h2' style={{ margin: 'auto 0 26px 0' }} content={t('editInvoice')} />
                <label>{t('label')}</label>
                <Input error={!invoiceData.name} id='name' type='text' value={invoiceData.name} onChange={onChange} disabled={isSaving} />
                <label>{t('dueTo')}</label>
                <DatePicker
                    id='due_date'
                    locale={locale}
